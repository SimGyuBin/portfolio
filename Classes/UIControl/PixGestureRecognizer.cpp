...
#define GESTURE_RECOGNIZER_LONGPRESS_THRESHOLD                          0.4f
...
PixGestureRecognizer::PixGestureRecognizer() : Ref()
{
	_gestureHandler = NULL;
}

PixGestureRecognizer::~PixGestureRecognizer()
{
	_gestureHandler = NULL;
}

bool PixGestureRecognizer::init()
{
	_gestureHandler = NULL;
...
PixSimpleGestureRecognizer::PixSimpleGestureRecognizer() : PixGestureRecognizer()
{
	_finger0 = NULL;
	_finger1 = NULL;
...
PixSimpleGestureRecognizer::~PixSimpleGestureRecognizer()
{
...
bool PixSimpleGestureRecognizer::init()
{
	PixGestureRecognizer::init();
...
bool PixSimpleGestureRecognizer::onTouchBegan(Touch* touch, Event* event)
{
	if (_finger0 == NULL)
	{
		_finger0 = touch;
		...
		if (_longPressEnabled && !_finger1)
		{
			...
			scheduler->unschedule(schedule_selector(PixSimpleGestureRecognizer::onLongPressActivate), this);
			scheduler->schedule(schedule_selector(PixSimpleGestureRecognizer::onLongPressActivate), this, 0, 0, _longPressThreshold, false);
...
void PixSimpleGestureRecognizer::onTouchMoved(Touch* touch, Event* event)
{
	...
	if (_finger0 && _finger1 && _finger0Moved && _finger1Moved)
	{
		...
		if (_pinchEnabled && fingersDistanceChange > _pinchFingersDistanceChangeTolerance)
		{
			...
			if (!_lastPinch)
			{
				_lastPinch = new PixGesturePinch();
				_lastPinch->setID(getNextPinchID());
			}
			_lastPinch->setTouch0(_finger0);
			_lastPinch->setTouch1(_finger1);
		...
		if (_rotationEnabled && fingersDistanceChange < _rotationFingersDistanceChangeTolerance)
		{
			...
			if (!_lastRotation)
			{
				_lastRotation = new PixGestureRotation();
...
void PixSimpleGestureRecognizer::onTouchEnded(Touch* touch, Event* event)
{
	if (_finger0 && touch->getID() == _finger0->getID())
	{
		...
		if (!_finger1)
		{
			...
			{
				// Prev tap
				PixGestureTap* prevTap = _lastTap;
				...
				if (threshold < _tapThreshold)
				{
					...
					PixGestureTap* lastTap = new PixGestureTap();
					...
					if (_tapEnabled)
					{
						...
						else if (_doubleTapEnabled && _tapRequiresDoubleTapRecognitionToFail)
						{
							...
							Scheduler* scheduler = Director::getInstance()->getScheduler();
							scheduler->unschedule(schedule_selector(PixSimpleGestureRecognizer::onTapActivate), this);
							scheduler->schedule(schedule_selector(PixSimpleGestureRecognizer::onTapActivate), this, 0, 0, _doubleTapInterval, false);
					...
					if (activateDoubleTap)
					{
						...
						PixGestureTap* gesture = new PixGestureTap();
			...
			else /* _finger0Moved == true */
			{
				...
				if (_swipeEnabled)
				{
					...
					{
						PixGestureSwipe* gesture = new PixGestureSwipe();
...
void PixSimpleGestureRecognizer::onTouchCancelled(Touch* touch, Event* event)
{
	if (_finger0 && touch->getID() == _finger0->getID())
	{
		CC_SAFE_RELEASE_NULL(_finger0);
...
void PixSimpleGestureRecognizer::onLongPressActivate(float time)
{
	// Long press recognition is applied on 1st finger only
    
    /*
     by gyubin 16.05.19
     안드로이드 터치 롱탭 오작동으로 _finger0Moved 주석처리 
     따로 onTouchBegan Location 과 롱탭의 Location 계산해서 사용할것
     */
    
	if (_longPressActivatingScheduled && _finger0 /*&& !_finger0Moved */)
	{
		PixGestureLongPress* gesture = new PixGestureLongPress();
		gesture->setID(getNextLongPressID());
//		gesture->setLocation(_finger0StartPt);
        gesture->setLocation(_finger0Pt);
...
void PixSimpleGestureRecognizer::onTapActivate(float time)
{
	PixGestureTap* lastTap = _lastTap;
...
// Get current time
double PixSimpleGestureRecognizer::getCurrentTime()
{
	timeval val;
